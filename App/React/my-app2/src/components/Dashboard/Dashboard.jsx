...
import useRealData from '../../hooks/useRealData'; // Cambiado a useRealData
import '../../styles/Dashboard.css';

const Dashboard = () => {
  const { currentData, historicalData, loading, error } = useRealData();
    ...
    return (
      <div className="dashboard dark-theme">
        <div className="loading-container">
          <h2>Cargando datos de sensores...</h2>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="dashboard dark-theme">
        <div className="error-container">
          ...
          <p>{error}</p>
          <p>Verifica que el servidor FastAPI esté ejecutándose en http://localhost:8000</p>
        </div>
      </div>
    );
  }
        ...
        <aside className="sidebar">
          ...
          <div className="status-card">
            ...
            <div className="status-list">
              <div className={`status-item ${!error ? 'online' : 'offline'}`}>
                <span className="status-dot"></span>
                <span>API: {!error ? 'Conectada' : 'Desconectada'}</span>
              </div>
